...
SlamClient::SlamClient(char* inifile)
	...
	,GoalDataReceiveCB(this, &SlamClient::GoalDataReceive)
	,ini_(inifile)
	,victimCount_(0)
	,victims_(0,0)
{
...
SlamClient::~SlamClient()
{
	...
	m_client.remHandler("Goal",&GoalDataReceiveCB);
...
void	SlamClient::register_to()
{
	...
	m_client.addHandler("Goal",&GoalDataReceiveCB);
	m_client.request("Mappa",200);
	m_client.request("Others",200);
	m_client.request("Victim",200,&nullPack);
	m_client.request("Goal",200,&nullPack);
}
...
//---------------------------------------------------------------------------
void	SlamClient::GoalDataReceive(ArNetPacket* packet)
{
}
...
void	SlamClient::SetGoal(DoublePair pos)
{
	ArTypes::Byte header=1;//1=set, 0=default heartbeat
	ArNetPacket pack;
	pack.byteToBuf(header);
	pack.doubleToBuf(pos.first);
	pack.doubleToBuf(pos.second);
	m_client.requestOnce("Goal",&pack);
	cout<<"Settato il nuovo goal: "<<pos.first<<"  "<<pos.second<<endl;
}