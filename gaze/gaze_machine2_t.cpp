...
/////////////////////////////////////////////////////////////////////////////
void gaze_machine2_t::create_devices_()
{
  //eye cameras
  eye_[gaze_machine2_t::left].reset(new core::opencv_lightweight_grabber_t(&sync_grabber_));
  eye_[gaze_machine2_t::right].reset(new core::opencv_lightweight_grabber_t(&sync_grabber_));
  
  //
  scene_[gaze_machine2_t::left].reset(new core::opencv_lightweight_grabber_t(&sync_grabber_));
  scene_[gaze_machine2_t::right].reset(new core::opencv_lightweight_grabber_t(&sync_grabber_));
...
//    <<std::endl;
    ...
    if( !eye_[left]->open(true) ) return false; 
    printf("--> Select EYE right\n");
    if( !eye_[right]->open(true) ) return false; 

    //scene left and right
    printf("--> Select SCENE left\n");
    if( !scene_[left]->open(true) ) return false; 
    printf("--> Select SCENE right\n");
    if( !scene_[right]->open(true) ) return false; 
...
{  
	//grab request
  sync_grabber_.grab_barrier.wait();
  //rpy
  ...
  sync_grabber_.retrieve_barrier.wait();

  //actually get images.
  ipl_eye_img_[left]    = eye_[left]->get_iplimage();
  ipl_eye_img_[right]   = eye_[right]->get_iplimage();

  ipl_scene_img_[left]  = scene_[left]->get_iplimage();
  ipl_scene_img_[right] = scene_[right]->get_iplimage();
}
/////////////////////////////////////////////////////////////////////////////
void gaze_machine2_t::get_dims_()
{
  ...
  eye_sz_[left]  = eye_[left]->image_size();
  eye_sz_[right] = eye_[right]->image_size();

  //
  scene_sz_[left] = scene_[left]->image_size();
  scene_sz_[right] = scene_[right]->image_size();
...
void gaze_machine2_t::show_loop()
{
  ...
  { 
      //Create threads
    boost::thread eleft (boost::bind(&opencv_lightweight_grabber_t::grab_loop, eye_[left].get() ));
	  boost::thread eright(boost::bind(&opencv_lightweight_grabber_t::grab_loop, eye_[right].get() ));
	  boost::thread sleft (boost::bind(&opencv_lightweight_grabber_t::grab_loop, scene_[left].get() ));
	  boost::thread sright(boost::bind(&opencv_lightweight_grabber_t::grab_loop, scene_[left].get() ));
    //
